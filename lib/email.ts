...
export async function sendSubscriptionReminders(): Promise<{ sent: number }> {
  ...
  for (const org of orgs) {
    ...
    const html = `<div style="font-family:sans-serif;max-width:520px;padding:20px">
      ...
      <p style="margin-top:14px"><a href="${process.env.NEXT_PUBLIC_APP_URL || "https://onecloudstay.com"}/settings" style="background:#4f46e5;color:#fff;padding:10px 18px;border-radius:8px;text-decoration:none">ต่ออายุแพ็กเกจ</a></p>
    </div>`;
